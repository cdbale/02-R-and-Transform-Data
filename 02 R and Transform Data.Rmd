...
- Our desires and emotions are shaped by wisdom and calmness
- We are humble and full of love
- We restrain anger, nurture patience, and treat others with tolerance, respect, and dignity

"Adorned with the Virtue of Temperance", Elder Ulisses Soares, Oct. 2025 General Conference.
...
Imagine we are consulting Patagonia. They provide a messy spreadsheet exported from the CRM (Customer Relationship Management) system. The data includes names, locations, and demographic details for thousands of customers. They also send you a huge file containing the transaction history for all the customers, but the only thing the two files have in common is the customer IDs. You are asked to find the 100 longest-standing customers, and figure out which 5 states brought in the most revenue last year. How would you do this? 
...
customer_data |> 
  ...
  # filter to just the ones who had the most transactions in February 2005
  filter(region == "West", feb_2005 == max(feb_2005))|> 
  # create a new column for age
  mutate(age = 2026 - birth_year) |>  
  # select only the age and February 2005 transactions
  select(age, feb_2005) |> 
...
Now that we've built up a lot of skills, let's try applying them together. Recall that you are consulting for Patagonia. Imagine the company wants a list of the 15 most valuable customers in the Midwest region. How should we define "valuable," and how would we come up with that list?
